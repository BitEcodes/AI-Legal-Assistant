...
import { Consultation } from "@/Entities/Consultation.jsx";
...
export default function ConsultationPage() {
  ...
  const loadConsultations = async () => {
    try {
      const res = await Consultation.list("-created_date", 10);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.query_title || !formData.detailed_question || !formData.category) return;
            ...
            {consultations.length === 0 ? (
              ...
              consultations.map((item) => (
                <Card key={item.id} className="shadow">
                  ...
                  {item.response && (
                    <CardContent className="bg-slate-50 border-t pt-3 pb-3">
                  ...
                  <CardContent className="border-t pt-3 text-xs text-slate-500 flex justify-between">
                    <span>
                      Submitted: {new Date(item.created_date).toLocaleDateString()}
                    </span>
                    <span>Query ID: #{item.id.slice(0, 8)}</span>
                  </CardContent>
                </Card>
              ))
            )}
